...
async function exportWeb(domain) {
  console.log("Exporting PWA web build...");
...
function patchWebIndex(webDistDir) {
  const indexPath = path.join(webDistDir, "index.html");
  if (!fs.existsSync(indexPath)) {
    console.warn("[web] index.html not found, skipping patch");
    return;
  }
  ...
  const themeColor = '<meta name="theme-color" content="#00d4aa" />';
  ...
  console.log("[web] index.html patched with manifest + SW");
}
